...
try:
    ...
    if not hasattr(db_utils, 'get_firestore_collection_ref') or \
       ...
       not hasattr(db_utils, 'get_all_users'):
...
# Lista fixa de opções de modal
MODAL_OPTIONS = ['Aéreo', 'Maritimo', 'Rodoviário', 'Consolidado']
...
@st.cache_data(ttl=60) # Ajustado TTL para 60s
def obter_processos_filtrados(
    status_filtro: Union[str, List[str]] = "Todos", # Pode ser uma string ou lista de strings
    termos_pesquisa: Optional[Dict[str, Any]] = None,
    limit: Optional[int] = None,
    start_after_doc_id: Optional[str] = None
) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    """
    Busca processos do Firestore aplicando filtros de status e termos de pesquisa, com suporte a paginação.
    Retorna uma tupla: (lista de processos, ID do último documento para a próxima paginação).
    """
    logger.info(f"obter_processos_filtrados: status_filtro={status_filtro}, termos_pesquisa={termos_pesquisa}, limit={limit}, start_after={start_after_doc_id} (Firestore).")
    ...
    try:
        ...
        if isinstance(status_filtro, str):
            status_list_to_filter = [status_filtro]
        else:
            status_list_to_filter = status_filtro

        # Lógica para Status_Arquivado
        if "Arquivados" in status_list_to_filter:
            query_firestore = query_firestore.where("Status_Arquivado", "==", "Arquivado")
            # Se "Arquivados" é o único filtro, não adicionamos Status_Geral
            if len(status_list_to_filter) > 1 and "Todos" not in status_list_to_filter:
                 # Se há outros status além de "Arquivados", filtramos também por eles
                general_statuses_excluding_archived = [s for s in status_list_to_filter if s != "Arquivados" and s != "Todos"]
                if general_statuses_excluding_archived:
                    query_firestore = query_firestore.where("Status_Geral", "in", general_statuses_excluding_archived)
        elif "Todos" not in status_list_to_filter:
            # Se não é "Todos" e nem "Arquivados", filtra pelos status gerais selecionados
            # E exclui explicitamente arquivados
            query_firestore = query_firestore.where("Status_Geral", "in", status_list_to_filter)
            query_firestore = query_firestore.where("Status_Arquivado", "in", [None, "Não Arquivado"])
        # Se status_filtro é "Todos" e não inclui "Arquivados", não adicionamos filtro de "Status_Arquivado" aqui,
        # permitindo que todos os processos (arquivados e não arquivados) sejam retornados pelo Firestore,
        # e a filtragem de arquivados será feita em memória mais tarde se necessário.
        ...
        # Ordem de preferência para paginação e exibição.
        # Campos de ordenação devem ser os mesmos em qualquer `start_after` se usados.
        query_firestore = query_firestore.order_by("Status_Geral").order_by("Modal").order_by("Processo_Novo")
        # Nota: Se você usar range queries (como em datas), as cláusulas order_by devem começar com o campo que está sendo rangeado.
        # Ajuste conforme seus índices do Firestore.

        # Lógica de Paginação (start_after)
        if start_after_doc_id:
            # Para usar start_after com um documento real, você precisa obter a referência do documento
            try:
                last_doc_snap = processos_ref.document(start_after_doc_id).get()
                if last_doc_snap.exists:
                    ...
                    logger.warning(f"obter_processos_filtrados: start_after_doc_id '{start_after_doc_id}' não encontrado. Iniciando do início.")
            except Exception as e:
                logger.error(f"obter_processos_filtrados: Erro ao obter documento para start_after: {e}. Iniciando do início.")

        if limit:
            query_firestore = query_firestore.limit(limit)

        docs = query_firestore.stream() # Obtém documentos com base nos filtros iniciais do Firestore

        processos = []
        last_fetched_doc_id = None
        for doc in docs:
            data = doc.to_dict()
            data['id'] = doc.id # Adiciona o ID do documento Firestore como 'id'
            # Mapeia Processos_Vinculados para LCL_Processos_Vinculados para consistência
            if 'Processos_Vinculados' in data and data['Processos_Vinculados'] is not None:
                data['LCL_Processos_Vinculados'] = data['Processos_Vinculados']
            else:
                data['LCL_Processos_Vinculados'] = []
            processos.append(data)
            last_fetched_doc_id = doc.id # Armazena o ID do último documento para a próxima página
        ...
        if termos_pesquisa:
            ...
            # A busca principal por Processo_Novo agora é um filtro de texto in-memory
            # pois as queries de Firestore não aceitam 'contains' ou 'like'.
            main_search_term_lower = termos_pesquisa.get('Processo_Novo', '').lower()

            for proc in processos:
                match = True
                # Primeiro, aplica a busca principal pelo Processo_Novo se houver um termo
                if main_search_term_lower:
                    if str(proc.get('Processo_Novo', '')).lower().find(main_search_term_lower) == -1:
                        match = False
                
                if not match: # Se não bateu na busca principal, pula para o próximo processo
                    continue

                # Em seguida, aplica os filtros de popup
                for col, termo in termos_pesquisa.items():
                    # Ignora filtros de data e Processo_Novo já considerados
                    if col in ["ETA_Recinto_Start", "ETA_Recinto_End", "Data_Registro_Start", "Data_Registro_End", "Processo_Novo"]:
                        continue
        ...
        return processos, last_fetched_doc_id
    except Exception as e:
...
def obter_todos_processos():
    """
    Busca todos os processos do Firestore. Esta função não deve ser usada para o carregamento principal da UI
    se houver muitos processos. Use 'obter_processos_filtrados' com paginação.
    Mantida para compatibilidade ou uso em cenários específicos onde todos os dados são realmente necessários (e poucos).
    """
    logger.warning("obter_todos_processos: Chamado. Considere usar 'obter_processos_filtrados' com paginação para melhor performance.")
    processos_ref = db_utils.get_firestore_collection_ref("followup_processos")
    if not processos_ref: 
...
def obter_processo_by_processo_novo(processo_novo: str):
    ...
    try:
        # NOVO DEBUG: Log the repr() of the document ID being sought
        logger.info(f"DEBUG_REPR: Searching Firestore for document ID: {repr(processo_novo)}")
...
def inserir_processo(dados: Dict[str, Any]) -> bool:
    ...
    # Garantir que LCL_Processos_Vinculados é uma lista antes de salvar
    # Também considera o nome do campo no DB 'Processos_Vinculados' ao salvar, se existir
    if 'LCL_Processos_Vinculados' in dados:
        if dados['LCL_Processos_Vinculados'] is None or not isinstance(dados['LCL_Processos_Vinculados'], list):
            dados['LCL_Processos_Vinculados'] = []
        dados['Processos_Vinculados'] = dados['LCL_Processos_Vinculados'] # Mapeia para o nome do campo no DB
    elif 'Processos_Vinculados' not in dados: # Se nenhum dos campos está presente, inicializa com lista vazia
        dados['LCL_Processos_Vinculados'] = []
...
def atualizar_processo(processo_id: str, dados: Dict[str, Any]) -> bool:
    ...
    # Mapeia LCL_Processos_Vinculados para Processos_Vinculados antes de enviar para o DB
    if 'LCL_Processos_Vinculados' in dados:
        if dados['LCL_Processos_Vinculados'] is None or not isinstance(dados['LCL_Processos_Vinculados'], list):
            dados['Processos_Vinculados'] = []
        else:
            dados['Processos_Vinculados'] = dados['LCL_Processos_Vinculados']
        del dados['LCL_Processos_Vinculados'] # Remove a chave temporária
...
def obter_nomes_colunas_db() -> List[str]:
    ...
    return [
        ...
        'Tipo_do_arquivo',
        'Conteudo_do_arquivo',
        # NOVOS CAMPOS DE CONSOLIDAÇÃO E LCL
        'Consolidado', # 'Sim' / 'Não'
        'LCL_Processos_Quantidade', # Quantidade de processos para LCL
        'Processos_Vinculados' # Lista de Processo_Novo IDs vinculados (nome real no DB)
    ]
