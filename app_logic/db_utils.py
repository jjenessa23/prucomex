def get_declaracoes_by_referencias(referencias):
    """
    Busca múltiplas declarações de importação (DI) por uma lista de referências de processo.
    Retorna um dicionário: {referencia: dados_DI_dict}
    """
    logger.info(f"db_utils.py: Buscando lote de declarações por referências: {referencias}")
    result = {}
    if db_firestore:
        declaracoes_ref = get_firestore_collection_ref("xml_declaracoes")
        if not declaracoes_ref:
            logger.error("db_utils.py: Falha ao acessar coleção 'xml_declaracoes' para busca batch.")
            return {ref: None for ref in referencias}
        # Limpa e padroniza as referências
        referencias_limpas = [_clean_reference_string(ref) for ref in referencias]
        # Firestore não suporta where IN para mais de 30 itens, então faz em lotes
        BATCH_SIZE = 30
        for i in range(0, len(referencias_limpas), BATCH_SIZE):
            batch_refs = referencias_limpas[i:i+BATCH_SIZE]
            try:
                query = declaracoes_ref.where(filter=firestore.FieldFilter("informacao_complementar", "in", batch_refs))
                docs = query.stream()
                for doc in docs:
                    data = doc.to_dict()
                    ref_key = data.get("informacao_complementar")
                    if ref_key:
                        result[ref_key] = data
            except Exception as e:
                logger.error(f"db_utils.py: Erro na busca batch de DI: {e}")
        # Garante que todas as referências requisitadas estejam no dicionário (mesmo que None)
        for ref, ref_limpa in zip(referencias, referencias_limpas):
            if ref_limpa not in result:
                result[ref] = None
            else:
                result[ref] = result[ref_limpa]
        return result
    else:
        logger.warning("db_utils.py: Firestore client não inicializado. Retornando dicionário vazio para batch DI.")
        return {ref: None for ref in referencias}

def get_fretes_internacionais_by_referencias(referencias):
    """
    Busca múltiplos fretes internacionais por uma lista de referências de processo.
    Retorna um dicionário: {referencia: dados_frete_dict}
    """
    logger.info(f"db_utils.py: Buscando lote de fretes internacionais por referências: {referencias}")
    result = {}
    if db_firestore:
        frete_ref = get_firestore_collection_ref("frete_internacional")
        if not frete_ref:
            logger.error("db_utils.py: Falha ao acessar coleção 'frete_internacional' para busca batch.")
            return {ref: None for ref in referencias}
        referencias_limpas = [_clean_reference_string(ref) for ref in referencias]
        BATCH_SIZE = 30
        for i in range(0, len(referencias_limpas), BATCH_SIZE):
            batch_refs = referencias_limpas[i:i+BATCH_SIZE]
            try:
                query = frete_ref.where(filter=firestore.FieldFilter("referencia_processo", "in", batch_refs))
                docs = query.stream()
                for doc in docs:
                    data = doc.to_dict()
                    ref_key = data.get("referencia_processo")
                    if ref_key:
                        result[ref_key] = data
            except Exception as e:
                logger.error(f"db_utils.py: Erro na busca batch de frete internacional: {e}")
        for ref, ref_limpa in zip(referencias, referencias_limpas):
            if ref_limpa not in result:
                result[ref] = None
            else:
        ...
        logger.warning("db_utils.py: Firestore client não inicializado. Retornando dicionário vazio para batch frete.")
        return {ref: None for ref in referencias}
import os
...
try:
    ...
    # Tenta carregar credenciais de st.secrets primeiro
    ...
    if "firestore_service_account" not in st_temp.secrets:
    ...
    firestore_secrets = st_temp.secrets["firestore_service_account"]
    ...
    logger.warning("db_utils.py: Streamlit não encontrado ou erro de importação. Tentando inicializar Firestore via variável de ambiente GOOGLE_APPLICATION_CREDENTIALS.")
    try:
...
COLLECTIONS_FIRESTORE = {
    ...
    "produtos": "produtos",
    "ncm_items": "ncm_items", # Esta coleção não é usada, mas mantida para compatibilidade
    "pagamentos_container": "pagamentos_container",
    "ncm_impostos_items": "ncm_impostos_items", # Esta é a coleção correta para NCM
        ...
        try:
            ...
            if not list(users_docs):
                ...
                all_screens_default = [
                    ...
                    "Cotação de Frete Internacional", "Inserir Cotação Agente" # Adicionadas as novas telas
                ]
                user_data = {
...
def create_tables():
    ...
    data_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', "data") # Ajustado o caminho
...
def save_dolar_cotacao_daily(tipo_dolar: str, valor: float, timestamp: str):
    """
    Salva a cotação de um tipo específico de dólar para o dia atual no Firestore.
    O ID do documento será 'YYYY-MM-DD_tipo_dolar' para garantir uma entrada por dia por tipo.
    :param tipo_dolar: O tipo de dólar (ex: 'abertura_compra', 'ptax_venda').
    :param valor: O valor da cotação.
    :param timestamp: O timestamp da cotação no formato ISO 8601 (ex: datetime.now().isoformat()).
    """
    if not st.session_state.get('firebase_ready', False):
        logger.error("Firebase não está pronto para salvar cotação do dólar.")
        return False
    ...
    try:
        ...
        doc_date_str = today_brasilia.strftime("%Y-%m-%d")

        # Cria um ID de documento único para o dia e tipo de dólar
        doc_id = f"{doc_date_str}_{tipo_dolar}"

        # Garante que o valor é uma string e substitui vírgula por ponto antes da conversão para float
        valor_str = str(valor).replace(',', '.')
        cotacao_data = {
            "tipo_dolar": tipo_dolar,
            "valor": float(valor_str), # Garante que o valor é float ao salvar
            "data": doc_date_str, # Salva a data no formato YYYY-MM-DD
            "timestamp": timestamp # Mantém o timestamp completo para ordenação
        }

        cotacoes_ref.document(doc_id).set(cotacao_data, merge=True) # Usa merge=True para atualizar se já existir
        logger.info(f"Cotação do dólar '{tipo_dolar}' salva/atualizada para {doc_date_str} no Firestore.")
        return True
    except Exception as e:
        logger.error(f"Erro ao salvar cotação do dólar '{tipo_dolar}' no Firestore: {e}")
        return False

def get_latest_dolar_cotacao_from_db():
    """
    Busca a última cotação de cada tipo de dólar salva no Firestore que não seja vazia.
    Retorna um dicionário com o tipo de dólar como chave e um dicionário com 'valor', 'data' e 'timestamp' como valor.
    Ex: {'abertura_compra': {'valor': 5.20, 'data': '2023-10-26', 'timestamp': '2023-10-26T10:30:00.000Z'}}
    """
    if not st.session_state.get('firebase_ready', False):
        logger.error("Firebase não está pronto para buscar cotações do dólar.")
        return {}

    db = st.session_state.db_firestore
    latest_valid_cotacoes = {} # Dicionário para armazenar as cotações válidas mais recentes
    
    # Definir os tipos de dólar que queremos buscar
    tipos_dolar = ['abertura_compra', 'abertura_venda', 'ptax_compra', 'ptax_venda']

    try:
        cotacoes_ref = db.collection("cotacoes_dolar")

        for tipo in tipos_dolar:
            # Busca as últimas N cotações para o tipo específico, ordenadas por timestamp decrescente.
            # Um limite maior que 1 é usado para permitir que a função encontre um valor válido
            # mesmo que a entrada mais recente seja inválida/N/A.
            query = cotacoes_ref.where(filter=firestore.FieldFilter('tipo_dolar', '==', tipo)).order_by("timestamp", direction=firestore.Query.DESCENDING).limit(10) # Busca até 10 entradas recentes
            docs = query.stream()

            found_valid_entry_for_type = False
            for doc in docs:
                cotacao_data = doc.to_dict()
                valor = cotacao_data.get('valor')
                data = cotacao_data.get('data') # A data já está no formato YYYY-MM-DD
                timestamp = cotacao_data.get('timestamp') # Pega o timestamp completo

                # Garante que o valor é uma string e substitui vírgula por ponto antes da conversão para float
                if valor is not None:
                    valor_str = str(valor).replace(',', '.')
                else:
                    valor_str = '' # Define como string vazia se for None para evitar erro em .strip()
                
                if valor_str.strip() != '': # Verifica se não está vazio após a limpeza
                    try:
                        valor_float = float(valor_str)
                        latest_valid_cotacoes[tipo] = {'valor': valor_float, 'data': data, 'timestamp': timestamp}
                        found_valid_entry_for_type = True
                        break # Encontrou a cotação válida mais recente para este tipo, sai do loop interno
                    except ValueError:
                        logger.warning(f"Valor '{valor_str}' para '{tipo}' não é um número válido. Ignorando e procurando o próximo mais recente.")
                        # Continua para o próximo documento no stream para este tipo
                else:
                    logger.info(f"Valor para '{tipo}' é nulo ou vazio. Procurando o próximo mais recente.")
                    # Continua para o próximo documento no stream para este tipo
            
            if not found_valid_entry_for_type:
                logger.warning(f"Nenhuma cotação válida encontrada no banco de dados para o tipo '{tipo}'.")
        
        logger.info(f"Últimas cotações VÁLIDAS do dólar do banco de dados: {latest_valid_cotacoes}")
        return latest_valid_cotacoes

    except Exception as e:
        logger.error(f"Erro ao buscar as últimas cotações do dólar no Firestore: {e}")
        return {}
        ...
        try:
            ...
            if user_doc.exists:
                ...
                if provided_password_hash == stored_password_hash:
                    ...
                    # Inclui todos os campos relevantes para user_info na sessão
                    return {
                        'id': user_doc.id,
                        ...
                        'is_freight_agent': user_data.get('is_freight_agent', False)
                    }
                else:
        ...
        try:
            ...
            for doc in users_ref.order_by("username").stream():
                ...
                users.append({
                    ...
                    'is_freight_agent': data.get('is_freight_agent', False)
                })
...
def get_user_by_id_or_username(identifier: Any) -> Optional[Dict[str, Any]]:
    """
    Busca um usuário pelo ID (username no Firestore). SOMENTE Firestore.
    """
    logger.info(f"db_utils.py: Buscando usuário por ID/username: {identifier}")
    if db_firestore:
        logger.info("db_utils.py: Usando Firestore para buscar usuário.")
        users_ref = get_firestore_collection_ref("users")
        if not users_ref:
            logger.error(f"db_utils.py: Falha ao acessar coleção 'users' no Firestore para buscar usuário.")
            return None
        try:
            doc_ref = users_ref.document(str(identifier)) # No Firestore, o username é o ID do documento
            doc = doc_ref.get()
            if doc.exists:
                data = doc.to_dict()
                logger.info(f"db_utils.py: Usuário '{identifier}' encontrado no Firestore.")
                return {
                    'id': doc.id, # Retorna o ID do documento (username)
                    'username': data.get('username'),
                    'is_admin': data.get('is_admin', False),
                    'allowed_screens': data.get('allowed_screens', []),
                    'email': data.get('email', ''), 
                ...
                logger.warning(f"db_utils.py: Usuário '{identifier}' não encontrado no Firestore.")
                return None
        except Exception as e:
            logger.error(f"db_utils.py: Erro ao buscar usuário '{identifier}' no Firestore: {e}")
            return None
    else:
        logger.warning(f"db_utils.py: Firestore client não inicializado ou desabilitado. Não é possível buscar usuário.")
    return None

def adicionar_ou_atualizar_usuario(user_id: Optional[str], username: str, password_hash: str, is_admin: bool, allowed_screens: List[str], email: Optional[str] = None, is_freight_agent: Optional[bool] = None) -> bool:
    """
    ...
    # Estes campos são opcionais e usados APENAS na vinculação de agente
    if email is not None:
        user_data["email"] = email
    if is_freight_agent is not None:
        user_data["is_freight_agent"] = is_freight_agent
...
def atualizar_senha_usuario(user_id: Any, new_password: str, username: str) -> bool:
    ...
    if db_firestore:
        ...
        logger.warning(f"db_utils.py: Firestore client não inicializado ou desabilitado. Não é possível atualizar senha.")
        success_firestore = False # Set to false if Firestore not ready
    ...
    if db_firestore:
        ...
        try:
            ...
            for doc in ncm_impostos_ref.order_by("ncm_code").stream():
                data = doc.to_dict()
                itens.append({
                    'ID': doc.id, # Retorna o ID do documento Firestore como 'ID'
...
def get_declaracao_by_referencia(referencia: str) -> Optional[Dict[str, Any]]:
    ...
    if db_firestore:
        ...
        try:
            ...
            docs = declaracoes_ref.where(filter=firestore.FieldFilter("informacao_complementar", "==", query_val)).limit(1).get()
            for doc in docs:
                data = doc.to_dict()
                data['id'] = doc.id
...
def get_itens_by_declaracao_id(declaracao_id: Any):
    ...
    if db_firestore:
        ...
        try:
            ...
            docs = itens_ref.where(filter=firestore.FieldFilter("declaracao_id", "==", query_declaracao_id)).order_by("numero_adicao").order_by("numero_item_sequencial").stream()
    ...
    if db_firestore:
        ...
        if processo_dados_custo_ref and processo_contratos_cambio_ref:
            try:
                ...
                old_contracts = processo_contratos_cambio_ref.where(filter=firestore.FieldFilter("declaracao_id", "==", str(declaracao_id))).stream()
...
def get_process_cost_data(declaracao_id: Any):
    ...
    if db_firestore:
        ...
        try:
            ...
            contracts_data = []
            contract_docs = processo_contratos_cambio_ref.where(filter=firestore.FieldFilter("declaracao_id", "==", str(declaracao_id))).stream()
...
def parse_xml_data_to_dict(xml_file_content: str) -> Tuple[Optional[Dict[str, Any]], Optional[List[Dict[str, Any]]]]:
    ...
    try:
        ...
        if match_referencia:
            referencia_extraida = match_referencia.group(1).strip()
...
def update_processo_di_link(processo_novo_ref: str, di_id_vinculada: str) -> bool:
    ...
    if db_firestore is None:
        return False, "Firestore client não inicializado. Não é possível atualizar processos."
        ...
        if declaracoes_ref_firestore and itens_ref_firestore and processo_dados_custo_ref and \
           processo_contratos_cambio_ref and frete_internacional_ref and processos_ref_firestore:
            try:
                ...
                docs_to_delete_itens = itens_ref_firestore.where(filter=firestore.FieldFilter("declaracao_id", "==", str(declaracao_id))).stream()
                ...
                contract_docs_to_delete = processo_contratos_cambio_ref.where(filter=firestore.FieldFilter("declaracao_id", "==", str(declaracao_id))).stream()
...
def inserir_ou_atualizar_produto(produto: Tuple[str, str, str, str]):
    ...
    if db_firestore:
        logger.info(f"db_utils.py: Usando Firestore para inserir/atualizar produto.")
...
def selecionar_produtos_por_ids(ids: List[str]):
    ...
    if db_firestore:
        ...
        for i in range(0, len(ids), 10):
            ...
            try:
                docs = produtos_ref.where(filter=firestore.FieldFilter(firestore.FieldPath.document_id(), 'in', batch_ids)).stream()
                for doc in docs:
...
def update_process_shipping_info(
    processo_novos_list: List[str],
    navio: Optional[str] = None,
    data_embarque: Optional[str] = None, # Formato YYYY-MM-DD
    eta_recinto: Optional[str] = None # Formato YYYY-MM-DD
) -> Tuple[bool, Optional[str]]:
    """
    Atualiza as informações de envio (Navio, Data_Embarque, ETA_Recinto) para múltiplos processos.
    Args:
        processo_novos_list: Lista de referências de processos (Processo_Novo) a serem atualizados.
        navio: Novo nome do navio. Se None, não será alterado.
        data_embarque: Nova data de embarque (ETD) no formato 'YYYY-MM-DD'. Se None, não será alterado.
        eta_recinto: Nova data de ETA no recinto no formato 'YYYY-MM-DD'. Se None, não será alterado.
    Returns:
        Uma tupla (bool, Optional[str]) indicando sucesso e uma mensagem de erro, se houver.
    """
    logger.info(f"db_utils.py: Iniciando atualização de informações de envio para processos: {processo_novos_list}")
    if db_firestore is None:
        return False, "Firestore client não inicializado. Não é possível atualizar processos."

    processos_ref = get_firestore_collection_ref("followup_processos")
    if not processos_ref:
        return False, "Falha ao acessar coleção 'followup_processos' no Firestore."

    updated_count = 0
    error_messages = []

    for processo_novo in processo_novos_list:
        cleaned_processo_novo = _clean_reference_string(processo_novo)
        try:
            doc_ref = processos_ref.document(cleaned_processo_novo)
            doc = doc_ref.get()

            if doc.exists:
                update_data = {}
                if navio is not None:
                    update_data['Navio'] = navio
                if data_embarque is not None:
                    update_data['Data_Embarque'] = data_embarque
                if eta_recinto is not None:
                    update_data['ETA_Recinto'] = eta_recinto

                if update_data:
                    doc_ref.update(update_data)
                    updated_count += 1
                    logger.info(f"db_utils.py: Processo '{cleaned_processo_novo}' atualizado com sucesso. Dados: {update_data}")
                else:
                    logger.info(f"db_utils.py: Nenhum dado para atualizar para o processo '{cleaned_processo_novo}'.")
            else:
                error_messages.append(f"Processo '{processo_novo}' não encontrado.")
                logger.warning(f"db_utils.py: Processo '{processo_novo}' não encontrado para atualização.")

        except Exception as e:
            error_messages.append(f"Erro ao atualizar processo '{processo_novo}': {e}")
            logger.error(f"db_utils.py: Erro ao atualizar processo '{processo_novo}': {e}", exc_info=True)

    if not error_messages and updated_count > 0:
        return True, None
    elif error_messages and updated_count == 0:
        return False, "; ".join(error_messages)
    elif error_messages and updated_count > 0:
        return True, f"Alguns processos foram atualizados, mas ocorreram erros: {'; '.join(error_messages)}"
    else:
        return False, "Nenhum processo foi atualizado e nenhum erro específico foi reportado."

def get_declaracoes_by_ids_batch(di_ids: List[str]) -> Dict[str, dict]:
    """Busca declarações por ID em lote e retorna um dicionário {id: dados}"""
    result = {}
    for di_id in di_ids:
        if not di_id:
            continue
        try:
            di_data = get_declaracao_by_id(di_id)
            if di_data:
                result[di_id] = di_data
        except Exception as e:
            logger.error(f"Erro ao buscar declaração ID {di_id}: {e}")
    return result

def get_declaracoes_by_referencias_batch(referencias: List[str]) -> Dict[str, dict]:
    """Busca declarações por referência em lote e retorna um dicionário {referencia: dados}"""
    result = {}
    for ref in referencias:
        if not ref:
            continue
        try:
            di_data = get_declaracao_by_referencia(ref)
            if di_data:
                result[ref] = di_data
        except Exception as e:
            logger.error(f"Erro ao buscar declaração para referência {ref}: {e}")
    return result

def get_fretes_internacionais_by_referencias_batch(referencias: List[str]) -> Dict[str, dict]:
    """Busca fretes internacionais por referência em lote e retorna um dicionário {referencia: dados}"""
    result = {}
    for ref in referencias:
        if not ref:
            continue
        try:
            frete_data = get_frete_internacional_by_referencia(ref)
            if frete_data:
                result[ref] = frete_data
        except Exception as e:
            logger.error(f"Erro ao buscar frete para referência {ref}: {e}")
    return result
