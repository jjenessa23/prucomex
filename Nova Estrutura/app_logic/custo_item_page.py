...
def perform_calculations(di_data, itens_data, expense_inputs, contracts_df):
    ...
    total_quantidade_itens_di = sum(item[5] for item in itens_data if item[5] is not None)
    ...
    for item_data in itens_data:
        qty = item_data[5] if item_data[5] is not None else 0
    ...
    for item_data in itens_data:
        ...
        ii_item_val_brl = vlmd_brl_item * (ii_perc_item if ii_perc_item is not None else 0.0)
        ipi_item_val_brl = (vlmd_brl_item + ii_item_val_brl) * (ipi_perc_item if ipi_perc_item is not None else 0.0)
        pis_item_val_brl = vlmd_brl_item * (pis_perc_item if pis_perc_item is not None else 0.0)
        cofins_item_val_brl = vlmd_brl_item * (cofins_perc_item if cofins_perc_item is not None else 0.0)
        icms_item_val = cif_unitario_item * (icms_perc_item if icms_perc_item is not None else 0.0)
        ...
        if sku_item and desc_mercadoria.startswith(sku_item):
            display_desc_mercadoria = re.sub(r'^{}\s*-\s*'.format(re.escape(sku_item)), '', desc_mercadoria).strip()

        itens_df_data.append({
            "ID": item_id,
            "Código ERP": st.session_state.item_erp_codes.get(item_id, codigo_erp_do_db if codigo_erp_do_db else ""),
            "NCM": _format_ncm(ncm_item),
            "SKU": sku_item if sku_item else "N/A",
            "Descrição": display_desc_mercadoria if display_desc_mercadoria else "N/A",
            "Quantidade": _format_float(qty, 0),
    ...
    for index, row in itens_df.iterrows():
        item_id_df = row["ID"]
        # Precisamos de uma forma de mapear o ID do DataFrame de volta para o item_data original
        # para obter o numero_adicao. Em Streamlit, o DataFrame é reconstruído.
        # Uma solução é garantir que o numero_adicao esteja no DataFrame desde o início,
        # ou passá-lo para a função perform_calculations.
        # Por simplicidade aqui, vamos re-iterar sobre itens_data ou usar um mapeamento.
        # Para fins de demonstração, vamos usar um mock de mapeamento ou inferir.
        
        # Para este mock, vamos simplesmente aplicar o fator médio geral se não tivermos a adição
        # Ou, idealmente, o numero_adicao deveria ser uma coluna no itens_df_data
        
        # Uma forma mais robusta seria:
        # if 'num_adicao' in itens_df.columns:
        #     num_adicao_for_item = row['num_adicao']
        #     if num_adicao_for_item in adicao_fator_medio:
        #         itens_df.loc[index, "Fator por Adição"] = _format_float(adicao_fator_medio[num_adicao_for_item], 4)
        # else:
        #     # Fallback se num_adicao não estiver no DataFrame
        #     itens_df.loc[index, "Fator por Adição"] = _format_float(sum(adicao_fator_medio.values()) / len(adicao_fator_medio) if adicao_fator_medio else 0.0, 4)
    ...
    total_row_data["Quantidade"] = _format_float(itens_df["Quantidade"].apply(lambda x: float(str(x).replace(',', '.'))).sum(), 0)
    total_row_data["Peso Unitário"] = _format_weight_no_kg(itens_df["Peso Unitário"].apply(lambda x: float(str(x).replace(' KG', '').replace(',', '.'))).sum())
...
def _generate_excel_for_cadastro(di_data, itens_data, item_erp_codes):
    ...
    headers = ["COD", "SKU", "Descrição", "NCM", "Referência"]
    ws.append(headers)
...
def _import_excel_for_cadastro(uploaded_file, itens_data):
    ...
    try:
        ...
        try:
            col_cod = header_row.index("COD")
            col_sku = header_row.index("SKU")
        except ValueError:
            st.error("O arquivo Excel deve conter as colunas 'COD' e 'SKU'.")
            return 0

        updates_count = 0
        for row_idx in range(2, ws.max_row + 1):
            erp_code = ws.cell(row=row_idx, column=col_cod + 1).value
            sku_from_excel = ws.cell(row=row_idx, column=col_sku + 1).value

            if erp_code is None:
                continue

            found_item_id = None
            for item_tuple in itens_data:
                # If it's a sqlite3.Row object, convert to tuple/list first
                if isinstance(item_tuple, sqlite3.Row):
                    item_tuple = tuple(item_tuple)

                if item_tuple[11] == sku_from_excel: # item_tuple[11] é o sku_item
                    found_item_id = item_tuple[0] # item_tuple[0] é o id do item
                    break
            
            if found_item_id:
                # Atualizar o código ERP no banco de dados e na sessão
                ...
                logger.warning(f"SKU '{sku_from_excel}' do Excel não encontrado nos itens da DI carregada. Item não atualizado.")
...
def _generate_cover_pdf(di_data, total_para_nf, process_totals, contracts_df):
    """Gera a capa do processo em PDF com base nos dados fornecidos."""
    if not di_data:
    ...
    # Placeholder para imagem da capa (assumindo que não temos o arquivo no ambiente Streamlit)
...
def show_page():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Total do Processo", "Impostos", "Despesas", "Contratos"])
    ...
    with tab3:
        ...
        if expenses_display:
            ...
            # Campos editáveis
            st.session_state.expense_inputs['afrmm'] = st.number_input("AFRMM", value=st.session_state.expense_inputs['afrmm'], format="%.2f", key="afrmm_input")
            st.session_state.expense_inputs['siscoserv'] = st.number_input("SISCOSERV", value=st.session_state.expense_inputs['siscoserv'], format="%.2f", key="siscoserv_input")
            st.session_state.expense_inputs['descarregamento'] = st.number_input("DESCARREGAMENTO", value=st.session_state.expense_inputs['descarregamento'], format="%.2f", key="descarregamento_input")
            st.session_state.expense_inputs['taxas_destino'] = st.number_input("TAXAS DESTINO", value=st.session_state.expense_inputs['taxas_destino'], format="%.2f", key="taxas_destino_input")
            st.session_state.expense_inputs['multa'] = st.number_input("MULTA", value=st.session_state.expense_inputs['multa'], format="%.2f", key="multa_input")
    ...
    with tab4:
        ...
        if st.session_state.di_data:
            ...
            # Usando st.data_editor para edição tabular dos contratos
            st.session_state.contracts_df = st.data_editor(
                st.session_state.contracts_df,
                column_config={
            ...
            # O botão de salvar já está na aba de despesas e salva ambos.
            # if st.button("Salvar Contratos (Simulado)", key="save_contracts"):
            #     st.success("Contratos salvos (simulado)! No aplicativo real, isso seria gravado no banco de dados.")
        else:
    ...
    if st.session_state.di_data:
        ...
        st.info("Carregue os dados da DI para visualizar os detalhes dos itens.")
