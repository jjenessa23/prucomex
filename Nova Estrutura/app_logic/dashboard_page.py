...
def show_dashboard_page():
    ...
    if conn_check:
        ...
        st.error(f"Não foi possível conectar ao banco de dados de Follow-up em: {db_manager.get_followup_db_path()}")
    ...
    if not df.empty:
        ...
        with col_pie:
            ...
            if 'Status_Geral' in df.columns and not df['Status_Geral'].empty:
                ...
                # Using Altair for a proper pie chart
                chart = alt.Chart(status_counts).mark_arc(outerRadius=120).encode(
                    theta=alt.Theta(field="Quantidade", type="quantitative"),
                    color=alt.Color(field="Status_Geral", type="nominal", title="Status"),
                ...
                text = chart.mark_text(radius=140).encode(
                    ...
                    order=alt.Order("Quantidade", sort="descending"),
                    color=alt.value("black") # Define a cor do texto para preto para melhor visibilidade
                )
        ...
        with col_bar:
            ...
            if not df_valid_previsao.empty:
                ...
                st.info("Nenhum processo com 'Previsão na Pichau' para exibir.")
    else:
    ...
    end_date = today + timedelta(days=days_option - 1) # Ajustado para incluir hoje + (N-1) dias
    ...
    if not df.empty and 'Data_Registro_dt' in df.columns:
        # Filter for valid dates within the selected number of days
        filtered_df_for_summary = df[
            (df['Data_Registro_dt'].notna()) &
            (df['Data_Registro_dt'].dt.date >= today) & # Compare date only
            (df['Data_Registro_dt'].dt.date <= end_date) # Compare date only
        ].copy()
    ...
    if not df.empty and 'Data_Registro_dt' in df.columns:
        ...
        for i in range(days_option): # Ajustado para incluir hoje + (N-1) dias
            ...
            daily_summary[current_date] = {'frete': 0.0, 'impostos': 0.0}
        ...
        for index, row in df.iterrows():
            # Use .dt.date to get Python date objects for direct comparison with `current_date`
        ...
        for i in range(0, len(sorted_daily_summary), cols_per_row):
            ...
            for j, (date, values) in enumerate(current_row_data):
                with cols[j]:
