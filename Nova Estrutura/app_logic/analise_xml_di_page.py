...
from db_utils import (
    ...
    connect_db,             # Para verificar a existência da DI
    get_db_path
)
    ...
    with st.container():
        with st.form(key="edit_di_before_save_form"): # Removido use_container_width aqui
            ...
            with tab_dados_di:
                ...
                with col2:
                    edited_di_data['informacao_complementar'] = st.text_input("Referência", value=di_data.get('informacao_complementar', ''))
            ...
            with col_save:
                if col_save.form_submit_button("Salvar no Banco de Dados"):
                    ...
                    if save_parsed_di_data(edited_di_data, itens_data): # Usa a nova função de salvar
                        ...
                        st.session_state.show_edit_popup_before_save = False
                        st.session_state.xml_declaracoes_data = get_all_declaracoes() # Recarrega a tabela principal
                        st.rerun()
                    else:
...
def show_page():
    ...
    # Exibe o pop-up de edição antes de salvar, se ativado
    _display_edit_popup_before_save()
    # Exibe o pop-up de itens, se ativado
    ...
    # Botões de ação
    # Mover o "Importar XML DI" para cima dos demais botões
    uploaded_file = st.file_uploader("Importar XML DI", type=["xml"], key="upload_xml_di")
        ...
        if di_data_parsed:
            ...
            if conn:
                ...
                if cursor.fetchone():
                    ...
                    conn.close()
                    _open_edit_popup_before_save(di_data_parsed, itens_data_parsed) # Corrigido o nome da função
            else:
                st.error("Não foi possível conectar ao banco de dados para verificar a existência da DI.")
        uploaded_file = None # Limpa o uploader para permitir nova importação
        st.rerun() # Força rerun para limpar o uploader e evitar reprocessamento

    # Botões de ação restantes em 2 colunas (removida a coluna do "Ver Itens")
    # Removendo o botão "Ver Itens da DI" da página principal
    col2 = st.columns(1)[0] # Apenas uma coluna para o botão "Editar DI Selecionada"
    ...
    # Carrega os dados na primeira vez ou quando a página é recarregada
    if 'xml_declaracoes_data' not in st.session_state:
        # Ao carregar, converte para lista de dicionários e adiciona a coluna de seleção
        raw_data = get_all_declaracoes()
        st.session_state.xml_declaracoes_data = []
        for row in raw_data:
            # Converte sqlite3.Row para dicionário usando os nomes das colunas do DB
            row_dict = dict(row)
            # Mapeia para os nomes de exibição esperados pelo DataFrame
            mapped_row = {
                "ID": row_dict['id'],
                ...
                "_Selecionar_DI": False # Adiciona a coluna de checkbox
            }
    ...
    # Se o DataFrame estiver vazio, inicializa com as colunas esperadas
    if df_display.empty:
    ...
    if not df_display.empty:
        # Captura o resultado do st.data_editor diretamente
        st.data_editor(
            df_display,
            ...
            column_config={
                ...
                "ID": st.column_config.NumberColumn("ID", format="%d", disabled=True, width="small"), # Alterado para NumberColumn e desabilitado
            ...
            on_change=_handle_declarations_table_change # Adicionado on_change aqui
        )
    ...
    with st.container():
        with st.form(key=f"edit_di_form_{declaracao_id_db}"):
            ...
            with tab_dados_di:
                ...
                with col2:
                    edited_data['informacao_complementar'] = st.text_input("Referência", value=declaracao_dict.get('informacao_complementar', ''))
            ...
            with col_save:
                if col_save.form_submit_button("Salvar Alterações"):
                    if update_declaracao(declaracao_id_db, edited_data):
                        ...
                        st.error("Falha ao atualizar a Declaração de Importação.")
