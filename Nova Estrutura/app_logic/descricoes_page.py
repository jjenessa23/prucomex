...
def show_page():
    ...
    with st.expander("Ações Principais"):
        ...
        with col_add:
            ...
            if st.button("Adicionar Novo Produto"):
                ...
                # O rerun abaixo vai reexibir o formulário
        with col_import:
    ...
    if st.session_state.selected_produto_id is not None or st.session_state.get('open_form_button_clicked', False):
        with st.form(key="produto_form"):
            ...
            produto_id_input = st.text_input(
                ...
                key="produto_id_input"
            )
            nome_input = st.text_input(_COLS_MAP_PRODUTOS['nome']['text'], value=initial_data.get('nome_part', ''), key="nome_input")
            desc_input = st.text_area(_COLS_MAP_PRODUTOS['desc']['text'], value=initial_data.get('descricao', ''), key="desc_input")
            ncm_input = st.text_input(_COLS_MAP_PRODUTOS['ncm']['text'], value=_format_ncm(initial_data.get('ncm', '')), key="ncm_input") # Corrigido typo
            ...
            with col_cancel:
                if col_cancel.form_submit_button("Cancelar"):
                    ...
                    st.session_state.open_form_button_clicked = False # Fecha o formulário
            st.rerun() # Força a re-execução para atualizar a UI (fechar formulário, etc.)
    ...
    if not df_display.empty:
        ...
        if selected_rows and selected_rows['selection']['rows']:
            ...
            st.session_state.selected_produto_id = df_display.iloc[selected_index]['id_key_erp']
            
            col_edit_selected, col_delete_selected = st.columns(2)
            with col_edit_selected:
                if st.button("Editar Selecionado (acima)", key="edit_selected_from_table"):
            ...
            with col_delete_selected:
                if st.button("Excluir Selecionado (acima)", key="delete_selected_from_table"):
                    if st.session_state.selected_produto_id:
                        if st.warning(f"Deseja realmente excluir o produto '{st.session_state.selected_produto_id}'?"):
                            if delete_produto_from_db(st.session_state.selected_produto_id):
                                st.session_state.selected_produto_id = None
                                st.session_state.open_form_button_clicked = False # Fecha o formulário
                                st.rerun()
